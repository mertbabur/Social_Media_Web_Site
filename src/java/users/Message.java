...
public class Message {
    ...
    public ArrayList<Message> findMessages(String userEmail, int friendid) {
        ArrayList<Message> messages = new MessageRepository().findMessagesFromFriend(userEmail, friendid);
        return messages;
    }
    ...
    public String moveToChatWithFriendPage(Message message, String userEmail) {
        int userBeanId = new UserRepository().findUserId(userEmail);
        if (userBeanId == message.fromUserId) {
            toUserId = message.toUserId;
        }
        else if(userBeanId == message.toUserId){
            toUserId = message.fromUserId;
        }
        return "chatWithFriendPage?faces-redirect=true";
    }
    
    public String deleteChat(int fromid, int toid){
        new MessageRepository().deleteChat(fromid, toid);
